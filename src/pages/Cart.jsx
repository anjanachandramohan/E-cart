  ...
  return (
      ...
      <table className='table border shadow'> 
        ...
        <tbody>
          {cartArray?.map((item,index)=>(
            ...
            <td>{index+1}</td>
            <td>{item.title}</td>
            <td><img width={'100px'} height={'100px'} src={item.image} alt='no image'></img></td>
            <td>Rs {item.price}</td>
    ...
    <div className='col-lg-4 d-flex justify-content-center align-items-center flex-column'>
      <div className='border shadow p-5'>
        ...
        <h5 style={{overflowY:'hidden'}}>Total no of products : <span className='text-primary fw-bolder fs-3'>{cartArray.length}</span></h5>
        <h4 style={{overflowY:'hidden'}}>Total Price : Rs {total}</h4>
